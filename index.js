...
client.connect((err) => {
  console.log(err);
  const serviceCollection = client.db("CPS").collection("service");
  const orderCollection = client.db("CPS").collection("orders");
  const reviewCollection = client.db("CPS").collection("reviews");
  const adminCollection = client.db("CPS").collection("admin");
...
app.get('/', (req, res) => {
  res.send('Hello!! Welcome to CPS Studio')
})
